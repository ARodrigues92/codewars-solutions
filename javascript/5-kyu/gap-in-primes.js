...
function gap(g, m, n) {
  const primes = [];
  for (let i = m; i <= n; i += 1) {
    const prime = isPrime(i, n);

    if (prime) {
      primes.push(prime);
      const firstPairIndex = primes.indexOf(prime - g);
      if (firstPairIndex !== -1) {
        const secondPairIndex = primes.indexOf(prime);
        if (secondPairIndex - firstPairIndex === 1) {
          return [primes[firstPairIndex], primes[secondPairIndex]];
        }
      }
    }
  }
